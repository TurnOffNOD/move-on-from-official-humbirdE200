...
(
  ...
  output reg LED_mon_clk32768,
  output reg LED_mon_boardclkin,

  input wire fpga_rst,//FPGA_RESET-T6
  input wire mcu_rst,//MCU_RESET-P20
  input wire jtag_rst_for_mpA7lite, // jtag debugger rst signal is output from jtag debugger, and it is fully connected with mcu_rst on DDR200T, add it.
...
//  );
  ...
  reg [7:0] counter;
  always @(posedge clk_8388 or negedge ck_rst)
  begin
    counter = (!ck_rst) ? 0 : ((counter +1) % 244);
    CLK32768KHZ = (!ck_rst) ? 0 :
                  ((counter ==0) ? ~CLK32768KHZ : CLK32768KHZ);
  end
  
  reg [15 -1: 0] clk_32768_mon2LED_counter;
  always @(posedge CLK32768KHZ or negedge ck_rst)
  begin
    clk_32768_mon2LED_counter = (!ck_rst) ? 0 : ((clk_32768_mon2LED_counter +1) % 32768);
    LED_mon_clk32768 = (!ck_rst) ? 1 :
                  ((clk_32768_mon2LED_counter ==0) ? ~LED_mon_clk32768 : LED_mon_clk32768);
  end

  reg [26 -1: 0] board_clkin_mon2LED_counter;
  always @(posedge clk_16M or negedge ck_rst)
  begin
    board_clkin_mon2LED_counter = (!ck_rst) ? 0 : ((board_clkin_mon2LED_counter + 1) % 16000000);
    LED_mon_boardclkin = (!ck_rst) ? 0 :
                         ((board_clkin_mon2LED_counter ==0) ? ~LED_mon_boardclkin : LED_mon_boardclkin);
  end
  ...
  (
    ...
    .clk_out1(clk_8388) // getting 32768Hz*256=8.388608MHz mid signal
  );

  assign ck_rst = fpga_rst & mcu_rst & jtag_rst_for_mpA7lite;
